...
This report covers \texttt{sella\_hip} IRC on \texttt{gad\_dt003} (Eckart, force\_norm gate)
...
all at 2000 steps, and adds the matched-fmax convergence gate. Kept for record.}}\end{center}
\vskip0.5em
\maketitle
\thispagestyle{empty}
...
local gate. Right: independent IRC verdict on every sample. The two
...
IRC TOPO rate tracks the gate closely at low noise (both $\approx$94\%)
but the IRC rate actually drops \emph{slower} than the gate --- at
150\,pm GAD's local gate says 71.3\% converged, while IRC-TOPO on
...
local gate. Right: independent IRC verdict. Sharp drop after 100\,pm on
both sides --- the IRC rate actually drops \emph{faster} than the gate,
...
25.7\% at 200\,pm. Unlike GAD, the IRC-TOPO rate tracks the gate
tightly or falls \emph{below} it at high noise --- 49\% IRC-TOPO at
150\,pm (vs 49.7\% gate), 24\% IRC-TOPO at 200\,pm (vs 25.7\% gate).
...
% =================================================================
\FloatBarrier
\section{The Gate-vs-IRC Sample Overlap}
% =================================================================

\emph{How well does the local convergence gate predict IRC success on
the same sample?} We cross every sample against two independent
booleans: local-gate-passed and IRC-TOPO-intended.
...
\begin{subfigure}[t]{0.48\textwidth}
  ...
  (local-gate pass) $\times$ (IRC-TOPO pass).}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
...
\end{subfigure}
\caption{\textbf{The ``gate only'' and ``IRC only'' categories
quantify disagreement.} For GAD, at 200\,pm, 28 samples pass the gate
but fail IRC (gate over-claims); 9 fail the gate but pass IRC (gate
...
Noise & both & gate only & IRC only & neither & both & gate only & IRC only & neither \\
\midrule
...
\textbf{Agreement.} The gates agree with IRC-TOPO on most samples
...
Filtering the IRC to only the samples that passed the local gate
