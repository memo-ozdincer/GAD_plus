...
The headline sweep restricts to TSs that pass the local gate
...
local gate is actually doing meaningful work and gives a preview of
...
trajectories existed there). No local-gate filter applied. Same
...
\textbf{Interpretation of the gate's marginal value.}
\begin{itemize}[nosep,leftmargin=1.5em]
\item At low noise (10--50\,pm) the gate barely moves the needle:
...
\item The gate's value ramps steeply with noise: at 100\,pm it saves
...
\item This calibrates the proposed ``IRC-as-convergence-criterion''
      direction: at low noise the local gate and an IRC gate will
...
\textbf{Sella's local gate is critical at high noise.} The TOPO drop
