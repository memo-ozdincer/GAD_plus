...
# ────────────────────────────────────────────────────────────────────────
# F2.  Reactant @ 0pm bar chart (TS conv at 3 thresholds)
# ────────────────────────────────────────────────────────────────────────
def reactant_data():
    rows = []
    # Sella libdef cart+Eckart (canonical name in this report)
    sf = con.execute(f"SELECT * FROM read_parquet('{RUNS}/test_reactant/sella_carteck_libdef/summary_*.parquet')").df()
    n = len(sf)
    rows.append({
        "config": "Sella cartesian Eckart untuned Hess.Freq.=1",
        "family": "Sella",
        "n":  n,
        "fmax_005":  100 * ((sf["final_n_neg"] == 1) & (sf["final_fmax"] < 0.05)).sum() / n,
        "fmax_023":  100 * ((sf["final_n_neg"] == 1) & (sf["final_fmax"] < 0.023)).sum() / n,
        "fmax_010":  100 * ((sf["final_n_neg"] == 1) & (sf["final_fmax"] < 0.01)).sum() / n,
        "is_partial": False,
    })

    # Plain GAD from reactant — pull final step of each traj
    for cfg in [("gad_dt003_fmax", "GAD dt=0.003"), ("gad_dt005_fmax", "GAD dt=0.005")]:
        path = f"{RUNS}/test_reactant/{cfg[0]}/traj_*.parquet"
        traj_files = glob.glob(path)
        if not traj_files:
            continue
        df = con.execute(f"""
            WITH src AS (SELECT * FROM read_parquet('{path}', filename=true)),
                 last AS (SELECT filename, MAX(step) AS mx FROM src GROUP BY filename)
            SELECT s.sample_id, s.force_max, s.n_neg
            FROM src s JOIN last l ON s.filename=l.filename AND s.step=l.mx
        """).df()
        nn = len(df)
        rows.append({
            "config": cfg[1],
            "family": "plain GAD",
            "n":  nn,
            "fmax_005":  100 * ((df["n_neg"] == 1) & (df["force_max"] < 0.05)).sum() / nn,
            "fmax_023":  100 * ((df["n_neg"] == 1) & (df["force_max"] < 0.023)).sum() / nn,
            "fmax_010":  100 * ((df["n_neg"] == 1) & (df["force_max"] < 0.01)).sum() / nn,
            "is_partial": nn < 287,
        })

    # GAD projected, 300-step cap, from starting_geom_300 — only force_norm
    sg = con.execute(f"SELECT * FROM read_parquet('{RUNS}/starting_geom_300/summary_start_reactant_*.parquet')").df()
    nn = len(sg)
    rows.append({
        "config": "GAD projected (300 steps, force_norm)",
        "family": "plain GAD",
        "n":  nn,
        "fmax_005":  100 * ((sg["final_n_neg"] == 1) & (sg["final_force_norm"] < 0.05)).sum() / nn,
        "fmax_023":  100 * ((sg["final_n_neg"] == 1) & (sg["final_force_norm"] < 0.023)).sum() / nn,
        "fmax_010":  100 * ((sg["final_n_neg"] == 1) & (sg["final_force_norm"] < 0.01)).sum() / nn,
        "is_partial": False,
    })
    ...
    for sub, tag, log_path in hybrid_cells:
        ...
        rows.append({
            ...
            "family": "hybrid",
            "n":  nn,
            "fmax_005":  100 * ((df["final_n_neg"] == 1) & (df["final_force_max"] < 0.05)).sum() / nn,
            "fmax_023":  100 * ((df["final_n_neg"] == 1) & (df["final_force_max"] < 0.023)).sum() / nn,
            "fmax_010":  100 * ((df["final_n_neg"] == 1) & (df["final_force_max"] < 0.01)).sum() / nn,
            "is_partial": nn < 287,
        })
    return pd.DataFrame(rows)

rdf = reactant_data()
rdf.to_csv(f"{CSV}/reactant_0pm_2026_05_16.csv", index=False)

# Bar chart: methods on x, grouped bars for 3 thresholds
fig, ax = plt.subplots(figsize=(15, 6.2))
methods = list(rdf["config"])
x = np.arange(len(methods))
w = 0.27
for i, (col, val) in enumerate([("fmax_005", "$F_\\mathrm{max}<0.05$"),
                                ("fmax_023", "$F_\\mathrm{max}<0.023$"),
                                ("fmax_010", "$F_\\mathrm{max}<0.01$")]):
    bars = ax.bar(x + (i-1)*w, rdf[col], width=w, label=val,
                  color=plt.cm.viridis(0.2 + 0.3*i),
                  edgecolor="black", linewidth=0.5)
    for b, v in zip(bars, rdf[col]):
        ax.text(b.get_x() + b.get_width()/2, v + 1.5, f"{v:.0f}",
                ha="center", va="bottom", fontsize=10)
labels = []
for cfg, n, partial in zip(rdf["config"], rdf["n"], rdf["is_partial"]):
    short = SHORT.get(cfg, cfg)
    if partial:
...
fig.tight_layout()
fig.savefig(f"{OUT}/fig_reactant_bar.pdf", bbox_inches="tight")
fig.savefig(f"{OUT}/fig_reactant_bar.png", bbox_inches="tight", dpi=140)
plt.close(fig)
print("Wrote fig_reactant_bar")
