...
\title{\Large\textbf{GAD vs Sella on HIP --- 3-method focused benchmark + step-size analysis}\\[4pt]
\normalsize 2026-04-28 (revision of 2026-04-20), Transition1x train, 300 samples $\times$ 6 noise levels, 2000 steps,
...
$n_{\text{neg}}{=}1 \wedge f_{\max}{<}0.01$ gate (canonical) plus an
...
(1) Under the matched fmax gate, \textbf{Sella Cartesian beats GAD by $\sim$4 pp at low noise}
...
inflated by GAD's looser force\_norm gate.
...
\begin{tabular}{lll}
\toprule
Name & Dynamics & Convergence gate \\
\midrule
...
\textbf{All methods now share the same matched $f_{\max}{<}0.01$ gate.} The earlier-reported
\texttt{gad\_dt003} numbers (force\_norm-gated) are kept as a sub-section in §\ref{sec:fmax_vs_fnorm}
...
\subsection{Three-method comparisons (headline)}

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.495\textwidth}
\includegraphics[width=\textwidth]{figures/fig_cmp_conv_3m.pdf}
\caption{\textbf{TS-converged rate vs.\ noise.} Denominator = 300
samples per cell. Each method uses its native force criterion
(Table in §1.2).}
\label{fig:cmp_conv}
\end{subfigure}
\hfill
...
labeled (R, P). Sella-internal 200\,pm point now included
(was missing in the 04-20 figure).}
\label{fig:cmp_irc}
\end{subfigure}
\caption{\textbf{Ranking is preserved across noise.} GAD Eckart (blue)
$>$ Sella cart+Eckart (red) $>$ Sella internal (purple), with gaps
widening at high noise. No-Eckart variants of each method dropped from
the headline figures (within $\pm$3\,pp of their Eckart counterparts at
all noise levels --- see appendix).}
\label{fig:main_cmp}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_irc_intended_grouped_3m.pdf}
\caption{\textbf{IRC TOPO-intended, grouped bar chart.} Same data as
Fig.~\ref{fig:cmp_irc} but easier to read magnitudes off. The crossover
at 100\,pm and the GAD lead at 150/200\,pm are both visible at a glance.}
\label{fig:irc_grouped}
\end{figure}

\begin{center}
\textbf{TS-converged rate (matched $f_{\max}{<}0.01$, \% of 300)} \\[0.3em]
...
by step-budget effects (see §\ref{sec:eckart_brief}).

\newpage
\clearpage
\subsection{GAD step-size over time (added 2026-04-28)}
\label{sec:gad_stepsize}

Per-step Cartesian displacement $|\Delta x|$ aggregated over all 300
\texttt{gad\_eckart\_fmax} trajectories per noise level, taken from
\texttt{disp\_from\_last} in the per-step parquets. Median (line) and
IQR (band).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_gad_stepsize_vs_step.pdf}
\caption{\textbf{GAD Eckart (dt=0.003) per-step displacement, by noise.}
Initial steps move 5--10\,m\AA, decaying to sub-m\AA{} as the
trajectory approaches the saddle. Higher noise lengthens the
``transient'' and pushes the plateau later. Note the IQR widens at
high noise --- some samples never settle within 2000 steps, consistent
with the 200\,pm timeout tail.}
\label{fig:gad_stepsize}
\end{figure}

\paragraph{Why this matters for the GAD-vs-Sella comparison.}
GAD with $dt{=}0.003$ takes per-step displacements of order
$10^{-3}$\,\AA{} once it leaves the transient. Sella's quasi-Newton
trust-region step is typically $10^{-2}$\,\AA{} (trust radius $\delta$
of $0.05$ multiplied by the QN unit step). \textbf{An order of magnitude
difference per step} explains a non-trivial portion of GAD's larger step
counts and may be partly responsible for the ``$\sim$2000-step budget
favors Sella'' artifact in §1.2. A larger-$dt$ GAD sweep
($dt\in\{5\!\times\!10^{-3},\,10^{-2}\}$) is queued to test this; results
to be appended once compute lands.

\paragraph{Sella step-size over time --- pending.}
Sella's per-step trajectory is not currently logged
(\texttt{trajectory=None} in \texttt{scripts/sella\_baseline.py}).
A small re-run with traj logging on (n=50, 100\,pm, $\sim$10 min on
MIG) is queued to produce a head-to-head step-size figure.

\newpage
\clearpage
\subsection{Per-method breakdown: TS-converged + IRC side-by-side}

For each method we show the raw TS-converged line chart next to the
IRC-TOPO proportional stacked bar chart (intended / half / unintended).
Denominator is always 300 samples per noise cell (\emph{not}
converged-only). No-Eckart variants of GAD and Sella Cartesian are
omitted here (within $\pm$3\,pp of their Eckart counterparts; full
breakdown in Appendix~\ref{sec:eckart_appendix}).

\begin{figure}[H]
\centering
\begin{subfigure}[t]{0.495\textwidth}
\includegraphics[width=\textwidth]{figures/fig_method_gad_eckart_conv_line.pdf}
\caption{GAD Eckart --- TS converged \\($n_{\text{neg}}{=}1 \wedge \|\mathbf{F}\|_{\text{mean}}{<}0.01$)}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.495\textwidth}
...
\caption{Sella cart+Eckart --- TS converged \\($n_{\text{neg}}{=}1 \wedge f_{\max}{<}0.01$)}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.495\textwidth}
...
$f_{\max}{<}0.01$ gate; both ``unlock'' and the previously-reported
``$1.33\times$ step efficiency'' \textbf{do not survive the corrected
analysis}.

\begin{center}
\textbf{Same-sample step efficiency, matched fmax gate.} \\[0.3em]
\begin{tabular}{rcccc}
\toprule
...
The convergence gate is $n_{\text{neg}}{=}1 \wedge f_{\max}{<}0.01$
...
comparing across mismatched gates: old GAD Eckart used force\_norm$<$0.01
(looser) while no-Eckart used fmax$<$0.01 (stricter). The looser gate
fires earlier in the trajectory, so Eckart "looked" faster. Under
matched gates the relationship inverts mildly.
...
\clearpage
\subsection{$f_{\max}$ vs $\|\mathbf{F}\|_{\text{mean}}$ gate --- brief comparison}
...
so fmax<0.01 is the strict gate. Switching from force\_norm to fmax produces a
...
\caption{\textbf{GAD Eckart convergence rate under the two gates.}
Same dynamics, same trajectories, two thresholds. The fmax gate is the
...
\textbf{IRC outcomes are insensitive to the choice of gate.} Validating
the same TS sets through \texttt{sella\_hip} IRC yields nearly identical
TOPO-intended rates whether we use the force\_norm-gated set or the
fmax-gated set: 92.7/92.7/89.7/86.3/69.7/47.9 vs 93.0/93.0/90.7/86.7/70.0/47.0.
Differences are within $\pm$1\,pp, well inside per-sample stochasticity.
This is the strongest argument that \emph{the choice of gate does not
...
Round~1 baseline, 300-step, $dt{=}0.01$). Both pools were gated on
$\|\mathbf{F}\|_{\text{mean}}{<}0.01$, so this is also a second data
point on how much the looser gate inflates downstream IRC.
...
These rates are $\sim$20--30\,pp below the fmax-gated GAD Eckart numbers
in the main body (92.7 / 89.7 / 86.3\,\% TOPO at 10/50/100\,pm). Both
backfill methods used the looser $\|\mathbf{F}\|_{\text{mean}}{<}0.01$
gate, so their TS pools include marginally-converged candidates that
pass force\_norm but not fmax; those drift on IRC. This independently
confirms the recommendation in the gate-comparison appendix:
$f_{\max}{<}0.01$ is the right gate for downstream IRC, not just for
