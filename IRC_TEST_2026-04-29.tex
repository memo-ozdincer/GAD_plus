...
\caption{Raw-convergence companion (Sella's library default gate fmax<0.05
...
Reporting under that gate (no saddle requirement) gives Sella some
...
\end{itemize}
The "right" gate is debatable. Below we use $f_{\max}{<}0.01$ as the
...
\newpage
% =================================================================
\section{Steps to convergence}
% =================================================================

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_test_steps_hist.pdf}
\caption{Histogram of total\_steps for converged samples
($f_{\max}{<}0.01 \wedge n_{\text{neg}}{=}1$, n=287). GAD takes
\emph{much} more steps than Sella (Sella is QN+trust, GAD is Euler).
Per-converged-TS GAD costs $\sim$50$\times$ more compute; but GAD
converges more samples at high noise.}
\end{figure}

\begin{center}
\textbf{Median compute (per-sample, includes failed runs)} \\[0.3em]
\begin{tabular}{l|cccccc}
\toprule
Method & 10 & 30 & 50 & 100 & 150 & 200 \\
\midrule
GAD dt=0.007 (5k) median calls   & ~150 & ~290 & ~440 & ~830 & ~1500 & ~2200 \\
Sella libdef median calls        & 4 & 6 & 7 & 9 & 11 & 13 \\
\bottomrule
\end{tabular}
\end{center}
...
\item dt=$10^{-3}$ at 5000-step cutoff is 6$\times$ slower than dt=0.007
  ...
  Smaller-dt is purely compute-wasteful for this gate.
...
budget 2000, gate $n_{\text{neg}}{=}1 \wedge f_{\max}{<}0.01$.
...
\texttt{threshold=0.01, gate=conv\_fmax\_pct}.
...
\item \textbf{Best Sella (Cartesian + Eckart):} \texttt{libdef} hyperparams
  ...
  step (\texttt{diag\_every\_n=1}), gate fmax<0.01.
...
$n=287$ test set under the gate $n_{\text{neg}}{=}1 \wedge f_{\max}{<}0.01$
eV/{\AA}. Source \texttt{analysis\_2026\_04\_29/threshold\_sweep.csv}.

\begin{center}\small
\textbf{Head-to-head best vs best (loose gate, fmax<0.01 $\wedge$ $n_{\text{neg}}{=}1$)} \\[0.3em]
\begin{tabular}{l|cccccc}
\toprule
method                                            & 10 & 30 & 50 & 100 & 150 & 200 \\
\midrule
...
with HIP H, same coordinate system, same gate).} Above table.
...
Median fail $f_{\max}$ = $0.046$ — only 5$\times$ the gate, not 50$\times$.
...
\paragraph{Cell legend.} \% conv (and total $n=50$ implicit). Strict gate
= $n_{\text{neg}}=1 \wedge f_{\max}<0.01$. Loose gate = $n_{\text{neg}}=1 \wedge f_{\max}<0.05$.

\begin{center}\small
\textbf{Strict gate} \\[0.3em]
...
\textbf{Reading the data:}
\begin{itemize}[leftmargin=1.5em]
\item GAD's lead persists at 300/500pm. Loose gate at 500pm: GAD 42\%, Sella 18\% ($+$24pp).
\item Both methods saturate at 1000pm+ (both at strict 0\%, only loose-gate signal). The 200pm in our headline grid was the right stress test.
...
no-Hessian partial). Same gate ($n_{\text{neg}}{=}1\,\wedge\,f_{\max}{<}0.01$),
...
\item \textbf{NR-GAD polish} (60151717): GAD dt=0.007 + spectral-NR refinement at $n_{\text{neg}}{=}1$, gates fmax<0.01 and fmax<1e-4. Tests whether NR breaks the GAD plateau (predicted: yes, RMSD should drop).
