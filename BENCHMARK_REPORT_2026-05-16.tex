...
Sella internal tuned Hess.Freq.=1           & 79.1 & 77.4 & 71.8 & 50.9 & 40.3$^{p_{67}}$ & 15.7$^{p_{134}}$ \\
Sella cartesian Eckart untuned Hess.Freq.=3 & \textbf{96.5} & \textbf{95.5} & \textbf{92.0} & 72.8 & 50.5 & 22.8$^{p_{171}}$ \\
\midrule
...
\textbf{Hybrid damped/undamped from reactant: 0\% conv at all thresholds
($n=51$ each)} — a clear mechanistic finding (see following paragraph).
...
\paragraph{Hybrid from reactant: 0\% (new finding, $n=51$).} The hybrid
trails plain GAD (40--54\% from reactant) by 40+ pp on this axis. Mechanism:
the hybrid's Newton step only fires when the Hessian's vibrational spectrum
has $n_\mathrm{neg}=1$ (eig-switch). Starting from the reactant, the
trajectory begins at $n_\mathrm{neg}=0$ (a local minimum) and the GAD
walking phase has to drag the geometry onto the saddle manifold before
Newton can activate. Within the 2000-step budget, the GAD walk doesn't
reach $n_\mathrm{neg}=1$ for any of the 51 samples observed. \emph{The
...
\textbf{Linear midpoint (R+P)/2}        & \textbf{52.7\%$^{p_{110}}$} & ---    \\
Reactant                                & 80.8\%   & 89.9\% \\
\bottomrule
\end{tabular}
\end{center}
...
200 pm & \textbf{27.2} & 22.8$^{p_{171}}$ & $-4.4$ & 107.0 / pending \\
\bottomrule
\end{tabular}
\end{center}
