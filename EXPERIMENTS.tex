...
%----------------------------------------------------------------------
\section*{7d. Round 4: Descent$\to$GAD One-Way Switch + Trajectory Analysis}

\textbf{Idea:} Instead of NR-GAD ping-pong (which oscillates), do a \textit{one-way} switch:
plain gradient descent ($\Delta\mathbf{x} = dt \cdot \mathbf{F}$, no Hessian inversion)
until $n_{\text{neg}} \leq$ threshold, then GAD permanently. No switching back. Same dt=0.005.

\begin{tabular}{l*{3}{r}l}
\toprule
Method & 50pm & 100pm & 200pm & Notes \\
\midrule
\textbf{gad\_small\_dt} & \textbf{91.3} & \textbf{86.7} & \textbf{51.3} & Pure GAD \\
descent\_then\_gad\_2  & 91.7 & 86.3 & 52.9$^\dagger$ & Switch at $n_{\text{neg}} \leq 2$ \\
descent\_then\_gad\_3  & 91.3 & 86.7 & 52.5$^\dagger$ & Switch at $n_{\text{neg}} \leq 3$ \\
\bottomrule
\end{tabular}

{\small $^\dagger$200pm still running ($\sim$238/300 samples).}

\textbf{Result:} Identical to baseline. The descent phase is 1--3 steps long (at dt=0.005,
$n_{\text{neg}}$ drops to $\leq$2 within 1--2 steps regardless of force type), too short to matter.

\textbf{Trajectory analysis} reveals \textit{why} NR-GAD ping-pong fails while this doesn't:

\vspace{0.3em}
\begin{tabular}{l*{4}{r}}
\toprule
Metric (100pm, 30 samples) & Pure GAD & NR-GAD & NR $\alpha$=0.1 & Desc$\to$GAD \\
\midrule
...
\textbf{Key finding:} NR-GAD's Newton step ($\mathbf{H}^{-1}\mathbf{g}$) \textbf{overshoots through
$n_{\text{neg}}=1$ into $n_{\text{neg}}=0$} (a minimum). Samples then spend $\sim$225 steps
trapped at a minimum, with $n_{\text{neg}}$ oscillating 105$\times$. Pure GAD and
Descent$\to$GAD \textit{never} visit $n_{\text{neg}}=0$ (0 steps). The problem is definitively
the Hessian inversion creating steps orders of magnitude too large along near-zero eigenvalue modes,
not the switching logic itself.
...
\textbf{NR switching hurts (trajectory proof)} & NR overshoots to $n_{\text{neg}}$=0, 225 steps trapped; gentle descent is fine but pointless \\
...
\textbf{Total optimizations:} $\sim$48,000 across 19 experiments (Rounds 1--4).

\textbf{Next steps:}
\begin{enumerate}[nosep,leftmargin=1.5em]
\item Multi-mode GAD: ascend along \textit{all} negative-eigenvalue modes (opposite of blend --- strengthen, not weaken)
