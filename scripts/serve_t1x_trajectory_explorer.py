...
<main class="wrap">
  <section class="card"><div class="section-title"><div><h2>Convergence rate versus noise</h2><p>Local TS convergence: projected index 1 and final force below 0.03 eV Å⁻¹. Each point is a matched method/noise cell.</p></div><div class="tabs"><button class="tab active" data-surface="gxtb">g-xTB</button><button class="tab" data-surface="lj7">LJ7</button></div></div><div id="convergencePlot" class="overview-plot"></div></section>
  <section class="card">
...
async function loadDashboard(){q('aggregateBody').innerHTML='<tr><td colspan="10">Loading…</td></tr>';q('examples').innerHTML='<div class="muted">Loading examples…</div>';try{let [a,e]=await Promise.all([getJSON('/api/aggregates?surface='+activeSurface),getJSON('/api/examples?surface='+activeSurface)]);renderConvergence(a.rows);renderAggregates(a.rows);renderExamples(e.rows)}catch(e){q('aggregateBody').innerHTML='<tr><td colspan="10" class="no">'+esc(e.message)+'</td></tr>';q('examples').innerHTML='<div class="no">'+esc(e.message)+'</div>';q('convergencePlot').textContent=e.message}}
function renderConvergence(data){let colors={competitive:'#0072B2',competitive_subspace:'#009E73',regular_gad:'#6b7280',sella:'#E69F00',ordinary_gad:'#6b7280',hard_gate:'#CC79A7',historical_lambda2:'#56B4E9',intrinsic:'#D55E00'},byMethod={};for(let r of data)(byMethod[r.method]??=[]).push(r);let methods=Object.keys(byMethod).sort((a,b)=>methodOrder.indexOf(a)-methodOrder.indexOf(b));let traces=methods.map(m=>{let rs=byMethod[m].sort((a,b)=>a.noise-b.noise);return {x:rs.map(r=>r.noise),y:rs.map(r=>100*r.local_count/r.starts),text:rs.map(r=>methodLabel(m)+'<br>noise '+r.noise+' '+r.noise_unit+'<br>'+r.local_count+'/'+r.starts+' local TS'),hovertemplate:'%{text}<br><b>%{y:.1f}%</b><extra></extra>',name:methodLabel(m),type:'scatter',mode:'lines+markers',line:{color:colors[m]||'#2563eb',width:2.5},marker:{size:8}}});Plotly.react('convergencePlot',traces,{margin:{l:60,r:20,t:14,b:55},hovermode:'closest',legend:{orientation:'h',y:1.13},xaxis:{title:'Cartesian noise ('+(data[0]?.noise_unit||'')+')',type:'linear',tickmode:'array',tickvals:[...new Set(data.map(r=>r.noise))].sort((a,b)=>a-b),automargin:true},yaxis:{title:'Local TS convergence (%)',range:[0,100],ticksuffix:'%',automargin:true},shapes:[{type:'line',x0:0,x1:1,xref:'paper',y0:50,y1:50,line:{color:'#98a2b3',dash:'dot',width:1}}]},{responsive:true,displaylogo:false,scrollZoom:true})}
