...
Sella internal tuned Hess.Freq.=1           & 79.1 & 77.4 & 71.8 & 50.9 & 41.7$^{p_{84}}$ & 17.0$^{p_{176}}$ \\
Sella cartesian Eckart untuned Hess.Freq.=3 & \textbf{96.5} & \textbf{95.5} & \textbf{92.0} & 72.8 & 50.5 & 23.9$^{p_{222}}$ \\
\midrule
...
\textbf{Hybrid damped/undamped from reactant: 1.3\%~/~5.1\% conv at $n=78$/79}
— case-dependent (see following paragraph).
...
\paragraph{Hybrid from reactant: 1--5\% (new finding, refined).} The hybrid
trails plain GAD (40--54\% from reactant) by 35+ pp on this axis. At $n=78$
the damped variant is at 1.3\% conv (one converged sample cluster: C3H5NO2,
samples 75-78). The undamped variant is at 5.1\%. Mechanism: the hybrid's
Newton step only fires when the Hessian's vibrational spectrum has
$n_\mathrm{neg}=1$ (eig-switch). Starting from the reactant, the trajectory
begins at $n_\mathrm{neg}=0$ (a local minimum) and the GAD walking phase
has to drag the geometry onto the saddle manifold before Newton can
activate. The result is \emph{case-dependent}: some molecular species
(C3H5NO2 in the test set) reach $n_\mathrm{neg}=1$ within $\sim$250 steps
and converge cleanly; harder species (C2H3N3O2, the first ~75 samples)
never reach $n_\mathrm{neg}=1$ in 2000 steps. \emph{The
...
\textbf{Linear midpoint (R+P)/2}        & \textbf{51.9\%$^{p_{133}}$} & ---    \\
Reactant                                & 80.8\%   & 89.9\% \\
\bottomrule
\end{tabular}
\end{center}
...
with 10000-step budget.} GAD (orange, $n=61$) plateaus at $\approx 0.01$
just like in the 2000-step sweep — \textbf{0\% reach $F_\mathrm{max}<0.005$
even at 5$\times$ budget}. Hybrid (green, $n=8$) and Sella (blue, $n=7$)
both reach $\sim$50--70\% at $F_\mathrm{max}<0.005$ via Newton landing.
...
200 pm & \textbf{27.2} & 23.9$^{p_{222}}$ & $-3.3$ & 107.0 / pending \\
\bottomrule
\end{tabular}
\end{center}
