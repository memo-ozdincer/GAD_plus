...
def random_cluster_geometry(
    ...
    for _ in range(max_attempts):
        candidate = (2.0 * torch.rand(3, generator=generator, dtype=torch.float64) - 1.0) * radius
        if torch.linalg.vector_norm(candidate) > radius:
            continue
        if not coords:
            coords.append(candidate)
        else:
            distances = torch.stack([torch.linalg.vector_norm(candidate - prev) for prev in coords])
    ...
    scales = sigma * (1.0 + torch.arange(n_atoms, dtype=torch.float64) / max(n_atoms - 1, 1))
