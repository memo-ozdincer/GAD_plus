...
This checkout is managed by `uv`. The lockfile is the source of truth: use `uv lock` after dependency/source changes. GADplus reuses the HIP checkout's virtual environment at `external/hip/.venv`; the HIP CUDA 12.1 stack is the base environment and GADplus adds only its extra local packages on top. Avoid `uv sync` from the GADplus root unless you intentionally want a separate root virtual environment.
...
Lock the GADplus dependency metadata, then install GADplus into the existing HIP
environment. Do not use plain `pip`; keep dependencies in `pyproject.toml` and
`uv.lock`.

```bash
cd /lustre/fs12/portfolios/nvr/projects/nvr_qualg_lmbm/users/anburger/GAD_plus && uv lock && uv pip install --python external/hip/.venv/bin/python -e '.[analysis]'
```
...
```bash
cd /lustre/fs12/portfolios/nvr/projects/nvr_qualg_lmbm/users/anburger/GAD_plus && source scripts/setup_cuda.sh && external/hip/.venv/bin/python -u scripts/method_single.py --method gad_dt003_fmax --noise 0.01 --n-samples 287 --n-steps 5000 --split test --output-dir runs/test_dtgrid/gad_dt003_fmax
```
