...
def create_app(database: Path) -> Flask:
    ...
    def step_histogram():
        ...
        con = connection()
        cur = con.execute(
            "SELECT method, noise, noise_unit, evaluations FROM runs WHERE surface=? AND local_ts AND evaluations>0",
            [surface],
        )
        rows = [dict(zip([d[0] for d in cur.description], row)) for row in cur.fetchall()]
        con.close()
