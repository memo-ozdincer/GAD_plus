...
<main class="wrap">
  ...
  <section class="card"><div class="section-title"><div><h2>Converged-candidate effort distribution</h2><p>Histogram of reported search evaluations/steps for local index-1 candidates only. Failed starts are excluded.</p></div></div><div class="overview-wrap"><button class="overview-fullscreen">Fullscreen</button><div id="stepHistogram" class="overview-plot"></div><div class="overview-lock" title="Open fullscreen to interact with this chart"></div></div></section>
...
function renderHistogram(rows){let colors={competitive:'#0072B2',competitive_subspace:'#009E73',regular_gad:'#6b7280',sella:'#E69F00',ordinary_gad:'#6b7280',hard_gate:'#CC79A7',historical_lambda2:'#56B4E9',intrinsic:'#D55E00'},by={};for(let r of rows)(by[r.method]??=[]).push(r.evaluations);let traces=Object.keys(by).sort((a,b)=>methodOrder.indexOf(a)-methodOrder.indexOf(b)).map(m=>({type:'histogram',x:by[m],name:methodLabel(m),opacity:.58,marker:{color:colors[m]||'#2563eb'},hovertemplate:methodLabel(m)+'<br>bin: %{x}<br>candidates: %{y}<extra></extra>'}));Plotly.react('stepHistogram',traces,{barmode:'overlay',margin:{l:62,r:20,t:14,b:55},legend:{orientation:'h',y:1.13},xaxis:{title:'Reported search evaluations / steps',type:'log',automargin:true},yaxis:{title:'Converged local candidates',automargin:true}},{responsive:true,displaylogo:false,scrollZoom:true})}
...
def create_app(database: Path) -> Flask:
    ...
    def step_histogram():
        ...
        con = connection()
        cur = con.execute(
            "SELECT method, evaluations FROM runs WHERE surface=? AND local_ts AND evaluations>0",
            [surface],
        )
        rows = [dict(zip([d[0] for d in cur.description], row)) for row in cur.fetchall()]
        con.close()
