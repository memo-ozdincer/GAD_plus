...
Same Sella cart+Eckart untuned d=1 with $n=287$ (FINAL), started from the
...
reactant for Sella: 46.7\% vs 80.8\% conv. Mechanism: the (R+P)/2 geometry
...
\emph{Follow-ups:} hybrid + plain GAD from-midpoint cells not yet generated;
add for a complete cross-family midpoint probe.

\paragraph{IRC TOPO for midpoint (NEW, $n=287$).} Sella IRC validation on
all 148 source-converged TS from the midpoint starting condition:
\textbf{134 / 148 per-converged match T1x reactant/product graphs}
(90.5\% per-conv). Full-N IRC TOPO \% = $134/287 = 46.7\%$, identical to
raw conv. \textbf{Recovery vs. raw conv: $-0.0$ pp} — Sella's midpoint
wins are essentially all real-basin (no wrong-saddle false positives on
this cell). RMSD-intended (strict 0.3 Å) = $85/148 = 57.4\%$ per-conv =
$29.6\%$ full-N. Source: \texttt{irc\_followup\_2026\_05\_16.csv} +
\texttt{runs/irc\_sella\_midpoint\_2026\_05\_16/merged\_irc.parquet}.
...
S internal tuned d=1           & \textbf{0.0} & 0.0 & \textbf{0.0} & \textbf{0.0} & --- & --- \\
...
GAD curves drop \emph{vertically} from 0.01 to 0.005 across every noise
level — plain GAD has essentially no samples in the
\SIrange{0.001}{0.005}{eV/\angstrom} band. Hybrid and Sella retain a
non-trivial fraction at 0.005 because Newton drives force down further.
...
\item \textbf{R1. Hybrid from reactant.} \textbf{DONE} (n=287, 2.1\% conv).
  The runner now takes \texttt{--start \{reactant, product, midpoint\}}.
  The 10k-budget follow-up confirmed the issue is geometric distance from
  reactant to saddle, not step count.
\item \textbf{R2. Hess.Freq.=3 at 200 pm.} \textbf{DONE} (n=287 pooled, 25.1\%).
  d=3 vs d=1 raw conv gap: -2.1 pp. IRC TOPO for 200 pm queued (SLURM 61164976).
\item \textbf{R3. Sella internal at 150 pm / 200 pm.} \textbf{DONE for 200 pm}
  (n=287 pooled, 13.9\%). 150 pm at n=172 partial (timed out, 35.5\%).
\item \textbf{R4. Tighter $F_\mathrm{max}$ via longer step budget.} \textbf{DONE}.
...
\item \textbf{R5. Mode-overlap-aware switching for hybrid.} Not started.
  ...
  plain GAD.
\item \textbf{R6 (new).} GAD + hybrid from midpoint @ 0 pm (cross-family
  midpoint probe to match the reactant story). Sella alone is 46.7\%; what
  about plain GAD and hybrid?
\end{itemize}
