...
def build_index(manifest: Path, lj_results: Path, database: Path, lj_sella_results: Path | None = None) -> None:
    ...
    if lj_results.is_file():
        ...
        for row in all_lj_rows:
            if row["method"] == "historical_lambda2":
                continue
...
<title>GADplus Explorer</title><script src="/plotly.min.js?v=2.35.2-r2"></script><style>
...
<main class="wrap">
  ...
  <section class="card"><div class="section-title"><div><h2>Converged-candidate effort by noise</h2><p>Log-binned histogram of reported search evaluations/steps for local index-1 candidates. Choose a noise level inside the chart; failed starts are excluded.</p></div></div><div class="overview-wrap"><button class="overview-fullscreen">Fullscreen</button><div id="stepHistogram" class="overview-plot"></div><div class="overview-lock" title="Open fullscreen to interact with this chart"></div></div></section>
...
const labels={competitive_subspace:'CS²-GAD',competitive:'Competitive GAD',regular_gad:'Regular GAD',sella:'Sella',ordinary_gad:'Regular GAD',hard_gate:'Hard gate',intrinsic:'λ₂ GAD'};
const methodOrder=['regular_gad','ordinary_gad','hard_gate','intrinsic','competitive','competitive_subspace','sella'];
...
async function loadDashboard(){q('aggregateBody').innerHTML='<tr><td colspan="10">Loading…</td></tr>';q('examples').innerHTML='<div class="muted">Loading examples…</div>';try{let [a,e,h]=await Promise.all([getJSON('/api/aggregates?surface='+activeSurface),getJSON('/api/examples?surface='+activeSurface),getJSON('/api/step-histogram?surface='+activeSurface)]);renderOverview(a.rows);renderHistogram(h.rows);bindFullscreen();renderAggregates(a.rows);renderExamples(e.rows)}catch(e){q('aggregateBody').innerHTML='<tr><td colspan="10" class="no">'+esc(e.message)+'</td></tr>';q('examples').innerHTML='<div class="no">'+esc(e.message)+'</div>';for(let id of ['localPlot','ircPlot','effortPlot','stepHistogram'])q(id).textContent=e.message}}
function renderOverview(data){let colors={competitive:'#0072B2',competitive_subspace:'#009E73',regular_gad:'#6b7280',sella:'#E69F00',ordinary_gad:'#6b7280',hard_gate:'#CC79A7',intrinsic:'#D55E00'},byMethod={};for(let r of data)(byMethod[r.method]??=[]).push(r);let methods=Object.keys(byMethod).sort((a,b)=>methodOrder.indexOf(a)-methodOrder.indexOf(b)),noise=[...new Set(data.map(r=>r.noise))].sort((a,b)=>a-b),axis={title:'Cartesian noise ('+(data[0]?.noise_unit||'')+')',type:'linear',tickmode:'array',tickvals:noise,automargin:true},make=(value,detail,suffix)=>methods.map(m=>{let rs=byMethod[m].filter(r=>value(r)!==null).sort((a,b)=>a.noise-b.noise);return {x:rs.map(r=>r.noise),y:rs.map(value),text:rs.map(r=>methodLabel(m)+'<br>noise '+r.noise+' '+r.noise_unit+'<br>'+detail(r)),hovertemplate:'%{text}<br><b>%{y:.1f}'+suffix+'</b><extra></extra>',name:methodLabel(m),type:'scatter',mode:'lines+markers',connectgaps:false,line:{color:colors[m]||'#2563eb',width:2.5},marker:{size:8}}}),shared={margin:{l:62,r:20,t:14,b:55},hovermode:'closest',legend:{orientation:'h',y:1.13},xaxis:axis},config={responsive:true,displaylogo:false,scrollZoom:true};Plotly.react('localPlot',make(r=>100*r.local_count/r.starts,r=>r.local_count+'/'+r.starts+' local index-1 candidates','%'),{...shared,yaxis:{title:'Local index-1 candidate recovery (%)',range:[0,100],ticksuffix:'%',automargin:true}},config);Plotly.react('ircPlot',make(r=>100*r.native_count/r.starts,r=>r.native_count+'/'+r.starts+' endpoint-validated events','%'),{...shared,yaxis:{title:'Endpoint-validated event recovery (%)',range:[0,100],ticksuffix:'%',automargin:true},shapes:[{type:'line',x0:0,x1:1,xref:'paper',y0:50,y1:50,line:{color:'#98a2b3',dash:'dot',width:1}}]},config);Plotly.react('effortPlot',make(r=>r.median_local_wall_time_s,r=>r.local_count&&r.median_local_wall_time_s!==null?r.median_local_wall_time_s.toFixed(1)+' s median wall time across '+r.local_count+' local TS candidates':'no timed local TS candidates',' s'),{...shared,yaxis:{title:'Median wall time to local TS (s)',type:'log',automargin:true}},config)}
function renderHistogram(rows){let colors={competitive:'#0072B2',competitive_subspace:'#009E73',regular_gad:'#6b7280',sella:'#E69F00',ordinary_gad:'#6b7280',hard_gate:'#CC79A7',intrinsic:'#D55E00'},noise=[...new Set(rows.map(r=>r.noise))].sort((a,b)=>a-b),methods=[...new Set(rows.map(r=>r.method))].sort((a,b)=>methodOrder.indexOf(a)-methodOrder.indexOf(b)),values=rows.map(r=>r.evaluations).filter(v=>v>0),lo=Math.max(1,Math.min(...values)),hi=Math.max(...values),nBins=18,ratio=Math.pow(hi/lo,1/nBins),edges=Array.from({length:nBins+1},(_,i)=>lo*Math.pow(ratio,i)),traces=[];for(let ni=0;ni<noise.length;ni++)for(let m of methods){let vals=rows.filter(r=>r.noise===noise[ni]&&r.method===m).map(r=>r.evaluations);if(!vals.length)continue;let counts=Array(nBins).fill(0);for(let value of vals){let index=value>=hi?nBins-1:Math.max(0,Math.min(nBins-1,Math.floor(Math.log(value/lo)/Math.log(ratio))));counts[index]++}let x=counts.map((_,i)=>Math.sqrt(edges[i]*edges[i+1])),custom=counts.map((count,i)=>[edges[i],edges[i+1],count,vals.length]);traces.push({type:'scatter',mode:'lines',line:{color:colors[m]||'#2563eb',width:2,shape:'hv'},fill:'tozeroy',fillcolor:(colors[m]||'#2563eb')+'20',x,y:counts.map(c=>c/vals.length),customdata:custom,name:methodLabel(m),visible:ni===0,hovertemplate:methodLabel(m)+'<br>noise '+noise[ni]+' '+rows[0].noise_unit+'<br>%{customdata[0]:.1f}–%{customdata[1]:.1f} steps/evals<br>%{customdata[2]} of %{customdata[3]} successes (%{y:.1%})<extra></extra>'})}let buttons=noise.map((level,ni)=>({label:level+' '+rows[0].noise_unit,method:'update',args:[{visible:traces.map(t=>t.hovertemplate.includes('noise '+level+' '))},{'title.text':'noise '+level+' '+rows[0].noise_unit}]}));Plotly.react('stepHistogram',traces,{margin:{l:68,r:20,t:48,b:55},title:{text:'noise '+noise[0]+' '+(rows[0]?.noise_unit||''),font:{size:14}},legend:{orientation:'h',y:1.18},updatemenus:[{buttons,direction:'down',x:1,xanchor:'right',y:1.18,yanchor:'top'}],xaxis:{title:'Reported search evaluations / steps',type:'log',automargin:true},yaxis:{title:'Fraction of successful runs / bin',tickformat:'.0%',rangemode:'tozero',automargin:true}},{responsive:true,displaylogo:false,scrollZoom:true})}
...
def create_app(database: Path) -> Flask:
    ...
    @app.get("/api/options")
    def options():
        con=connection(); result={key:[row[0] for row in con.execute(f"SELECT DISTINCT {key} FROM runs WHERE method <> 'historical_lambda2' ORDER BY {key}").fetchall()] for key in ("surface","method","noise")};con.close();return jsonify(result)
    ...
    def aggregates():
        ...
        cur = con.execute(
            """SELECT method, noise, noise_unit, count(*) AS starts,
               ...
               FROM runs WHERE surface=? AND method <> 'historical_lambda2'
    ...
    @app.get("/api/step-histogram")
    def step_histogram():
        surface = "analytic reduced LJ7" if request.args.get("surface") == "lj7" else "Transition1x / g-xTB"
        con = connection()
        cur = con.execute(
            "SELECT method, noise, noise_unit, evaluations FROM runs WHERE surface=? AND method <> 'historical_lambda2' AND local_ts AND evaluations>0",
            [surface],
        )
        rows = [dict(zip([d[0] for d in cur.description], row)) for row in cur.fetchall()]
        con.close()
    ...
    def examples():
        ...
        cur = con.execute(
            """SELECT * EXCLUDE(example_rank) FROM (
                   ...
                   FROM runs WHERE surface=? AND method <> 'historical_lambda2' AND has_trace
    ...
    def runs():
        ...
        clauses=["method<>'historical_lambda2'", "has_trace"] if include_failures else ["method<>'historical_lambda2'", "local_ts", "has_trace"]; values=[]
