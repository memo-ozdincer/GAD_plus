...
\toprule
Noise & Total & IRC ran & Gate failed & Intended & Half & Topology-half only / Unintended \\
\midrule
...
not code failure. The looser refined gate did recover one fully intended case at
...
\item Even 0 pm is only partially ``clean'' under the stricter pipeline: one
      intended case, a few half-intended cases, and many gate failures. This means
...
current rerun used the corrected post-refinement gate \(f_{\max}<0.006\).
...
\item The remaining failed-gate rows are now clustered around
...
So the main gain from the latest rerun came from the looser refined gate rather
...
\item The latest rerun under \(f_{\max}<0.006\) is better at 0 pm, but high-noise
      cases remain dominated by gate failures and unintended endpoints.
